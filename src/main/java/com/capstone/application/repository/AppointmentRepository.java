...
//	 Appointment findPreviousAppointmentById(int patientId);
	 ...
	 @Query(value = "select * from appointment where patient_id=2 order by appointment_id desc limit 1,1" ,nativeQuery=true)
	 ...
	 @Query(value = "select appointment_id from appointment where patient_id =?1",nativeQuery=true)
		...
		@Query(value = "select * from Appointment where patient_id=:id and acceptance='Accepted' order by date asc limit 1" ,nativeQuery=true)
